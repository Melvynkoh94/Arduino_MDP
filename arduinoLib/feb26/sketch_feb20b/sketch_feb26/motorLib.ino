...
const int TICKS[10] = {580, 1205, 1800, 2380, 3020, 3615, 4195, 4775, 5370, };
...
//const double kp = 7.35, ki = 1.25, kd = 0;  //whenever we increase kd, it goes to the right
...
void turnLeft(int angle) {
  ...
  for (i = 0; i < angle; i = i + 90) {
    initializeTick();
    initializeMotor_Start();
    while (tick_R < (TURN_TICKS_R) || tick_L - 15  < (TURN_TICKS_R)) {
      //    offset = computePID();
      if (myPID.Compute())
        md.setSpeeds(-(currentSpeed + speed_O), (currentSpeed - speed_O));
    }
...
void turnRight(int angle) {
  ...
  for (i = 0; i < angle; i = i + 90) {
    initializeTick();
    initializeMotor_Start();
    while (tick_R < (TURN_TICKS_R) || tick_L - 10 < (TURN_TICKS_R)) {
      //    offset = computePID();
      if (myPID.Compute())
        md.setSpeeds((currentSpeed + speed_O), -(currentSpeed - speed_O));
    }
...
//brakes the motor (for normal speed of 350)
