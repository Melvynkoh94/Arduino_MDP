...
const int TURN_TICKS_L = 624;       //change this left encoder ticks value to calibrate left turn 
const int TURN_TICKS_R = 605;       //change this right encoder ticks value to calibrate right turn 
...
const int TICKS[10] = {550, 1190, 1800, 2380, 3020, 3615, 4195, 4775, 5390, 0};  // for movement of each grid (e.g. move by 1 grid requires 580 ticks) - to be revised (thad)
  ...
  while(1){
    ...
    if(frontIR2 == 1){  //Obstacle is in front of Front Center sensor
      ...
      if(rightIR1 == 1 || rightIR2 == 1){
        flag = 0; //right side obstacle
        turnLeft(90);
        delay(500);
        while(rightIR1 == 1 || rightIR2 == 1){  //if the robot's right side still has an obstacle, keep moving forward 
          moveForward(1*10);           
        }
        turnRight(90);
        moveForward(4*10);
