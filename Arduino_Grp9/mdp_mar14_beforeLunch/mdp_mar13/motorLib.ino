...
int TURN_TICKS_R = 700;       //change this right encoder ticks value to calibrate right turn 
...
void alignRight(int noLeftRight) {
  ...
  double diff = (readRightSensor_2()+0.65) - (readRightSensor_1()+0.11); // -0.36. + 0.67
  /*while (true)
    {
      diff = (readRightSensor_2()-0.21) - (readRightSensor_1()-0.27);
      Serial.print(readRightSensor_2()+0.35);
      Serial.print(",");
      Serial.print(readRightSensor_1()+0.11);
      Serial.print(",");
      Serial.print(diff);
      Serial.print("\n");
    }*/
  ...
  while (abs(diff) > 0.2 && abs(diff) < 4)
  {
    ...
    */
    rotateRight(abs(diff*8), abs(diff)/diff*-1);
    diff = (readRightSensor_2()+0.35) - (readRightSensor_1()+0.11);
...
void alignFront() {
  ...
  int count = 0;
  double desiredDistanceSensor1 = -0.80;  // - 0.8 more
  double desiredDistanceSensor3 = -0.80;  // - 0.8 more
  double diffLeft = readFrontSensor_1() - desiredDistanceSensor1;
  double diffRight = readFrontSensor_3() - desiredDistanceSensor3;
