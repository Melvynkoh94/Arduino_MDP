...
int TURN_TICKS_R = 691;       //change this right encoder ticks value to calibrate right turn 
...
int TICKS[10] = {484 , 1095, 1680, 2285, 2910, 3470, 4050, 4775, 5390, 0};  // for movement of each grid
...
void alignRight(int noLeftRight) {
  ...
  double diff = (readRightSensor_2()-0.23) - (readRightSensor_1()-0.27); // -0.36. + 0.67
  /*while (true)
    {
      diff = (readRightSensor_2()-0.33) - (readRightSensor_1()-0.27);
      Serial.print(readRightSensor_2()-0.33);
      Serial.print(",");
      Serial.print(readRightSensor_1()-0.27);
      Serial.print(",");
      Serial.print(diff);
      Serial.print("\n");
    }*/
  ...
  while (abs(diff) > 0.3 && abs(diff) < 4)
  {
    ...
    */
    rotateRight(abs(diff*8), abs(diff)/diff*-1);
    diff = (readRightSensor_2()-0.23) - (readRightSensor_1()-0.27);
...
void alignFront() {
  ...
  int count = 0;
  double desiredDistanceSensor1 = -0.4;  // - 0.8 more
  double desiredDistanceSensor3 = -0.52;  // - 0.8 more
  double diffLeft = readFrontSensor_1() - desiredDistanceSensor1;
  double diffRight = readFrontSensor_3() - desiredDistanceSensor3;

  while ((abs(diffLeft) > 0.2 && abs(diffLeft) < 7)|| (abs(diffRight) > 0.2 && abs(diffRight) < 7))
  {   
...
void alignFrontStart() {
  ...
  double desiredDistanceSensor1 = -0.3;  // - 0.5 more
  double desiredDistanceSensor3 = -0.42;  // - 0.5 more
  double diffLeft = readFrontSensor_1() - desiredDistanceSensor1;
  double diffRight = readFrontSensor_3() - desiredDistanceSensor3;
