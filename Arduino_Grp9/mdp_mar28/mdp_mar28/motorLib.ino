...
const int MOVE_MAX_SPEED = 395;     //if target distance more than 60cm

const int TURN_MAX_SPEED = 395;     //change this value to calibrate turning. If the rotation overshoots, decrease the speed 
const int ROTATE_MAX_SPEED = 395;   //used in rotateLeft() and rotateRight()

int TURN_TICKS_L = 693;       //change this left encoder ticks value to calibrate left turn 
int TURN_TICKS_R = 693;       //change this right encoder ticks value to calibrate right turn
...
int TICKS[10] = {477, 1075, 1672, 2290, 2890, 3495, 4095, 4685, 5295, 0};  // for movement of each grid
...
void turnLeft(int angle) {
  ...
  else if (angle == 180)
  {
    initializeTick();
      initializeMotor_Start();
      while (tick_R < TURN_TICKS_L_180 || tick_L < TURN_TICKS_L_180) { // -15
        if (myPID.Compute())
          md.setSpeeds(-(currentSpeed + 2*speed_O), currentSpeed);
      }
      initializeMotorTurn_End();   //brakes the motor
...
void turnRight(int angle) {
  ...
  else if (angle == 180)
  {
    initializeTick();
      initializeMotor_Start();
      while (tick_R < TURN_TICKS_R_180 || tick_L < TURN_TICKS_R_180) { // -15
        if (myPID.Compute())
          md.setSpeeds((currentSpeed + 2*speed_O), -(currentSpeed));
      }
      initializeMotorTurn_End();   //brakes the motor
...
void alignRight(int noLeftRight) {
  ...
  int count = 0;
  double rightFrontOffset = 0.18;
  double rightBackOffset = -0.09;
  
  double diff = (readRightSensor_2()+rightFrontOffset) - (readRightSensor_1()+rightBackOffset); // Sensor2, adding allow it to move closer to the wall
  /*while (true)
    {
      diff = (readRightSensor_2()+rightFrontOffset) - (readRightSensor_1()+rightBackOffset); // Sensor2, adding allow it to move closer to the wall
      Serial.print(readRightSensor_2()+rightFrontOffset);
      Serial.print(",");
      Serial.print(readRightSensor_1()+rightBackOffset);
      Serial.print(",");
      Serial.print(diff);
      Serial.print("\n");
    }*/
  ...
  while (abs(diff) > 0.2 && abs(diff) < 4)
  {
    ...
    diff = (readRightSensor_2()+rightFrontOffset) - (readRightSensor_1()+rightBackOffset);
    count++;
  }
}
...
void rotateRight(int distance, int direct) {
  ...
  while (tick_R < distance) {
    if (myPID.Compute())
      md.setSpeeds(0, direct*(currentSpeed));
  }
  initializeMotor_End();
}
...
void rotateLeft(int distance, int direct) {
  ...
  while (tick_L < distance) {
    if (myPID.Compute())
      md.setSpeeds(direct*(currentSpeed), 0);
  }
  initializeMotor_End();
}
...
void alignFront() {
  ...
  /*while (true)
  {
    ...
    Serial.print(readFrontSensor_1() + desiredDistanceSensor1);
    Serial.print(",");
    Serial.print(readFrontSensor_3() + desiredDistanceSensor3);
    Serial.print("\n");
  ...
  while ((abs(diffLeft) > 0.2 && abs(diffLeft) < 10)|| (abs(diffRight) > 0.2 && abs(diffRight) < 10))
  {   
    if (abs(diffLeft) >= 0.2)
    {
